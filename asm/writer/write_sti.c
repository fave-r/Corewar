...
** Last update Sun Apr 13 19:49:54 2014 
*/
...
int		*write_sti(char *args, int *len, int fd, t_lab *lab)
{
  ...
  *len += write(fd, &val, 1);
  write_sti_arg(args_tab[0], len, fd, lab);
  write_sti_arg(args_tab[1], len, fd, lab);
  write_sti_arg(args_tab[2], len, fd, lab);
  return (len);
}
...
void		write_sti_arg(char *arg, int *len, int fd, t_lab *lab)
{
  ...
  if (arg[0] == 'r')
    ...
    {
      arg++;
      size_s = (arg[1] != ':') ? my_getnbr(arg) : find_good_lab(lab, arg);
